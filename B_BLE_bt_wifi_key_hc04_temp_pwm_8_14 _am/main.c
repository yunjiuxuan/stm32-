...
/* ===== 全局变量定义（声明在 main.h 中，供其�?.c 文件 extern 引用�?==== */
uint8_t _logflg = 0;
uint16_t data;
uint8_t cmd[50];
...
const version_info_t g_version = {
    VERSION_MAGIC, 1, 0, 20260814, "APP-B"
};


//运行时间计数（秒�? �?main.c 内部使用
static uint32_t g_run_sec = 0;
//app_task1 句柄 - �?main.c 内部使用
...
static void update_mode_led(GPIO_TypeDef* GPIOx, uint16_t pin, uint8_t state, uint8_t* toggle_flag)
{
	switch(state)
	{
		case STATUS_CONNECTED:
			GPIO_ResetBits(GPIOx, pin);   //常亮(低有�?
			break;
		case STATUS_ON:
			*toggle_flag ^= 1;
...
static void app_task1(void* pvParameters)
{
	...
	{ 
		...
		uint8_t temp = 24;//默认24�?
		if (dht11_getval(dht_buf) ==0  ){ 
		temp =	dht_buf[2]; 
		}		 

		/* ===== 倒计时递减(在OLED刷新之前执行,保证显示最新�? ===== */
		if(daojishi_state && daojishi_remain_sec > 0)
		{
			...
			{
				daojishi_state = 0;
				Process_Command(CMD_GEAR_OFF);  // 归零关风�?
...
int main(void)
{
	...
	//【重要】PA10 已被 BT_State_Init 用作蓝牙 STATE 输入�?
	//        不再恢复 USART1 �?RXNE 接收中断，否则会导致 USART1 空转
	//USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);  // 已禁�?
	...
	OLED_Display_Refresh(g_run_sec, 0, 0, 0);  //启动时未采集传感器，�?
...
//			  (TaskHandle_t*  )NULL);
	...
	//初始化按键与外部中断：必须在任务创建之后，确�?bt_handle/wifi_handle 非空�?
