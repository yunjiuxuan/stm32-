...
static int _OTA_InitESP8266(void)
{
    ...
    ESP8266_Send_AT("+++");
    delay_s(1);
    OLED_OTA_SetProgress(3, 100);   /* 3%: 退出透传 */
    ...
    OLED_OTA_SetProgress(6, 100);   /* 6%: AT测试 */

    ESP8266_Send_AT("AT+CIPCLOSE\r\n");
    ESP8266_Recv_ACK("OK", 2000);
    OLED_OTA_SetProgress(9, 100);   /* 9%: 关闭连接 */
    ...
    OLED_OTA_SetProgress(12, 100);  /* 12%: CWMODE设置 */

    char WIFI_PASS[64];
    sprintf(WIFI_PASS, "AT+CWJAP=\"%s\",\"%s\"\r\n", WIFI_USERNAME, WIFI_PASSWORD);
    OLED_OTA_SetProgress(15, 100);  /* 15%: WiFi连接中 */
    ESP8266_Send_AT(WIFI_PASS);
    ...
    OLED_OTA_SetProgress(18, 100);  /* 18%: WiFi连接成功 */

    ESP8266_Send_AT("AT+CIPMUX=0\r\n");
    ESP8266_Recv_ACK("OK", 2000);
    OLED_OTA_SetProgress(19, 100);  /* 19%: CIPMUX */
    ...
    OLED_OTA_SetProgress(20, 100);  /* 20%: ESP8266就绪 */
    return 0;
}
    ...
    BLE_PRINTF("[OTA] 4.6 开始接收 (三4KB缓冲静默模式, 下载完成后打印结果)...\r\n");
    {
        ...
        while (1)
        {
            ...
            if (g_ota_total_recv != last_total_recv)
            {
                ...
                /* OLED: 更新下载进度条 (每4KB缓冲满时更新) */
                OLED_OTA_SetProgress(g_ota_total_recv, g_bin_total_len);
            }
            else
            {
                idle_cnt++;
...
/* 扫描源分区Flash查找VERSION_MAGIC, 获取当前运行APP的纯整数版本号
 * 版本号存储约定: version_info_t.ver_major 字段直接存储服务器返回的纯整数(1,2,3...)
 *   ver_minor 字段忽略,不参与换算
 * 源分区 = 当前运行的APP分区 (非升级目标分区)
 * version_info_t内存布局(小端, 自然4字节对齐):
 *   word[0]=magic, word[1]=ver_minor<<16|ver_major, word[2]=build_date
 * 返回: 0=成功, -1=未找到
 */
static int _OTA_GetCurrentAppVersion(uint32_t *cur_version)
{
    uint32_t ota_req = RTC->BKP0R;
    uint32_t src_addr;
    volatile uint32_t *p;
    uint32_t i;
    const uint32_t SCAN_WORDS = 32768U / 4U;  /* 扫描前32KB */

    /* 源分区 = 当前运行的APP (非目标分区) */
    if (ota_req == OTA_UPGRADE_A)
    {
        src_addr = OTA_APP_B_ADDRESS;   /* 当前运行B, 升级到A -> 源=B */
    }
    else if (ota_req == OTA_UPGRADE_B)
    {
        src_addr = OTA_APP_A_ADDRESS;   /* 当前运行A, 升级到B -> 源=A */
    }
    else
    {
        /* 无明确OTA标志: 用BKP1R(上次运行记录)判断 */
        uint32_t last_boot = RTC->BKP1R;
        src_addr = (last_boot == OTA_LAST_BOOT_TO_B) ? OTA_APP_B_ADDRESS : OTA_APP_A_ADDRESS;
    }

    p = (volatile uint32_t *)src_addr;
    for (i = 0; i < SCAN_WORDS; i++)
    {
        if (p[i] != VERSION_MAGIC)
            continue;

        /* 匹配magic: 读取后续字段并验证 */
        {
            uint32_t w1 = p[i + 1];
            uint32_t w2 = p[i + 2];   /* build_date */
            uint32_t ver = w1 & 0xFFFFU;   /* ver_major 直接 = 纯整数版本号 */

            /* build_date 合理范围验证 (降低误判) */
            if (w2 >= 20200000U && w2 <= 20300000U)
            {
                *cur_version = ver;
                return 0;
            }
...
int OTA_HTTP_Test(void)
{
    ...
    /* OLED: "网络连接中" (ESP8266初始化+版本查询+JSON解析) */
    OLED_OTA_SetStage(OTA_STAGE_NET);

    BLE_PRINTF("[OTA] 步骤1: 初始化 ESP8266\r\n");
    if (_OTA_InitESP8266() != 0)
    {
        BLE_PRINTF("[OTA] 错误: ESP8266 初始化失败, 终止 OTA\r\n");
        OLED_OTA_SetResult(OTA_STAGE_ERR, OTA_ERR_ESP8266_INIT, 0);
        return -1;
    }
    OLED_OTA_SetProgress(20, 100);   /* 网络连接: 20% (ESP8266初始化完成) */

		////https://apis.bemfa.com/vb/api/v1/firmwareVersion?openID=3f2d5feab40ea3844d23321f165ab2db&topic=ota&deviceType=3
    BLE_PRINTF("[OTA] 步骤2: HTTP 查询固件版本 (bemfa API)\r\n");
    BLE_PRINTF("[OTA]   URL: http://%s%s\r\n", OTA_META_HOST, OTA_META_PATH);
    OLED_OTA_SetProgress(25, 100);   /* 25%: 发送HTTP元数据请求 */
    ret = _OTA_HTTP_Get(OTA_META_HOST, OTA_META_PORT, OTA_META_PATH,
                         meta_resp, OTA_META_RESP_SIZE, 5000, 0);
    if (ret != 0)
    {
        BLE_PRINTF("[OTA] 错误: 元数据请求失败\r\n");
        OLED_OTA_SetResult(OTA_STAGE_ERR, OTA_ERR_META_HTTP, 0);
        return -1;
    }
    OLED_OTA_SetProgress(55, 100);   /* 55%: 元数据HTTP响应完成 */
    ...
    OLED_OTA_SetProgress(65, 100);   /* 65%: 响应分段打印完成 */
    ...
    BLE_PRINTF("[OTA] 步骤3: 解析 JSON\r\n");
    {
        ...
        if (json_body == NULL)
        {
            ...
            OLED_OTA_SetResult(OTA_STAGE_ERR, OTA_ERR_META_PARSE, 0);
            return -1;
        }
    ...
    OLED_OTA_SetProgress(75, 100);  /* 75%: JSON解析完成 */
    ...
    /* ---- 版本对比: 当前APP版本 vs 服务器版本 (纯整数直接对比) ---- */
    {
        uint32_t cur_ver = 0;
        if (_OTA_GetCurrentAppVersion(&cur_ver) == 0)
        {
            BLE_PRINTF("[OTA] 当前APP版本: %lu, 服务器版本: %d\r\n",
                        (unsigned long)cur_ver, meta.version);
            if (meta.version > 0 && (uint32_t)meta.version <= cur_ver)
            {
                BLE_PRINTF("[OTA] 版本不大于当前版本, 无需升级, 显示提示并退回\r\n");
                OLED_OTA_ShowLatest();
                delay_s(3);
                return -1;
            }
            BLE_PRINTF("[OTA] 服务器版本较新, 继续升级流程\r\n");
        }
        else
        {
            BLE_PRINTF("[OTA] 警告: 未找到当前APP版本信息, 跳过版本对比\r\n");
        }
    }
    OLED_OTA_SetProgress(100, 100);  /* 100%: 版本对比完成, 网络连接阶段结束 */

		
    /* ---- 步骤4: 固件下载 (从 meta.url 写入 W25Q128) ---- */
    /* OLED: "获取升级中" (HTTP下载bin -> W25Q128) */
    OLED_OTA_SetStage(OTA_STAGE_DL);
    BLE_PRINTF("[OTA] 步骤4: 下载固件 (从元数据URL写入W25Q128)\r\n");
    ret = codeDownload(meta.url);
    if (ret != 0)
    {
        BLE_PRINTF("[OTA] 错误: 固件下载到 W25Q128 失败, OTA 终止\r\n");
        OLED_OTA_SetResult(OTA_STAGE_ERR, OTA_ERR_BIN_HTTP, 0);
        return -1;
    }
    ...
    /* OLED: "升级烧录中" (擦除+编程+校验内部Flash) */
    OLED_OTA_SetStage(OTA_STAGE_BURN);
...
int OTA_FlashAndJump(void)
{
    ...
    while (written < fw_size)
    {
        ...
        for (i = 0; (uint32_t)i < prog_chunk; i += 2)
        {
            ...
            if (FLASH_ProgramHalfWord(target_addr + written + i, halfword) != FLASH_COMPLETE)
            {
                ...
                OLED_OTA_SetResult(OTA_STAGE_ERR, OTA_ERR_FLASH_PROG, 0);
                FLASH_Lock();
                return -1;
            }
        }
        ...
        written += this_chunk;
        OLED_OTA_SetProgress(written, fw_size);
        if ((written % 8192) == 0)
        {
    ...
    if (fail)
    {
        ...
        OLED_OTA_SetResult(OTA_STAGE_ERR, OTA_ERR_VERIFY, 0);
        FLASH_Lock();
        return -1;
    }
    ...
    /* OLED: 显示"跳转中" + 100%进度 + 1.5秒延时让用户看到结果 */
    OLED_OTA_SetProgress(fw_size, fw_size);
    OLED_OTA_SetResult(OTA_STAGE_OK, 0, 0);
    delay_ms(1500);
...
erase_fail:
    ...
    OLED_OTA_SetResult(OTA_STAGE_ERR, OTA_ERR_FLASH_ERASE, 0);
    FLASH_Lock();
    return -1;
}
